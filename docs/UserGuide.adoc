...
Sorts the address book according to a selected parameter. Possible parameters are `name`, `phone`, `email`, and
`address` +
...
* `sort name` +
Sorts and prints out a name sorted list.

* `sort email` +
Sorts and prints out an email sorted list.
